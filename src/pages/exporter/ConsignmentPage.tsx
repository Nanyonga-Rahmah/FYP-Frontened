...
function ConsignmentPage() {
  ...
  useEffect(() => {
    const fetchConsignments = async () => {
      try {
        setLoading(true);
        const dummyData: Consignment[] = Array(6).fill({
          id: "CS-001",
          lotDetails: "LT-001, LT-002 (100kg)",
          submittedDate: "Nov 30, 2026",
          status: "Exported",
        });
        setConsignments(dummyData);
        setFilteredConsignments(dummyData);
      } finally {
        setLoading(false);
      }
    };
      ...
      <section className="px-20 py-10">
        ...
        <div className="rounded-t-md mt-12 pt-12">
          <div className="flex justify-between items-center px-6 pt-6">
            <h2 className="text-white text-xl font-semibold">Consignment History</h2>
            <div className="flex items-center gap-3">
              <Input
                placeholder="Search"
                value={searchTerm}
          ...
          <div className="grid grid-cols-3 gap-6">
            {filteredConsignments.map((item, idx) => (
              <div
                ...
                >
                <div className="flex items-center justify-between">
                  <span className="font-semibold text-sm text-black">{item.id}</span>
                ...
                <div className="text-sm flex flex-col gap-1 text-gray-700">
                  <p className="flex items-center gap-2">
                    <img src="/images/arabica.png" alt="Lots" className="w-4 h-4" />
                    {item.lotDetails}
                  </p>
                  <p className="flex items-center gap-2">
                ...
                <Button
                  ...
                  onClick={() => navigate(`/view-consignment-details/${item.id}`)}
                >
                  View Details
                </Button>
              </div>
