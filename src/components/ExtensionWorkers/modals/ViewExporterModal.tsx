...
export default function ViewExporterModal({
  exporter,
  onClose,
  onSubmit,
}: Props) {
  ...
  const handleApprove = async (notes: string) => {
    try {
      const response = await fetch(`${AllUsers}${exporter._id}/status`, {
        method: "PUT",
        headers: {
      ...
      <div className="fixed inset-0 z-50 bg-black/30 flex items-center justify-center">
        <div className="bg-white w-full max-w-xl rounded-md shadow-xl relative overflow-hidden">
          ...
          <div className="px-6 py-5 space-y-4 max-h-[75vh] overflow-y-auto">
            <div className="flex items-center justify-between">
              <p className="text-sm text-gray-500">
                KYC-{exporter._id.substring(0, 3)}
              </p>
              <span
                className={`text-white text-xs px-3 py-1 rounded-full ${
                  exporter.kyc.status.toLowerCase() === "approved"
                    ? "bg-[#3AB85E]"
                    : exporter.kyc.status.toLowerCase() === "rejected"
                      ? "bg-[#FF5C5C]"
                      : "bg-[#339DFF]"
                }`}
              >
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-4 text-sm">
              ...
              <div className="font-medium text-black text-right">
                {exporter.kyc.cooperativeLocation}
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                <div
                  ...
                  onClick={() => handleImageClick(exporter.kyc.nationalIdPhoto)}
                >
                  <img
            ...
            <div className="space-y-2">
              ...
              <div
                className="flex items-center bg-gray-100 border rounded px-3 py-2 cursor-pointer"
                onClick={() => handleImageClick(exporter.kyc.passportSizePhoto)}
              >
                <img
                  src="/images/image-placeholder.svg"
                  alt="icon"
            ...
            <div className="space-y-1 text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <p className="text-black">
                  {exporter.firstName} {exporter.lastName} #U-
                  {exporter._id.substring(0, 3)} created account
                </p>
                <span className="ml-auto text-gray-500">
                  {formatDateTime(exporter.kyc.submittedAt)}
                </span>
              </div>
              {exporter.kyc.processedAt && (
                <div className="flex items-center gap-2">
                  <div
                    className={`h-2 w-2 ${exporter.kyc.status.toLowerCase() === "approved" ? "bg-green-500" : "bg-red-500"} rounded-full`}
                  ></div>
                  <p className="text-black">
                    {exporter.kyc.status.toLowerCase() === "approved"
                      ? "Approved"
                  ...
                  <span className="ml-auto text-gray-500">
                    {formatDateTime(exporter.kyc.processedAt)}
                  </span>
                </div>
              )}
            </div>

            {exporter.kyc.adminNotes && (
          ...
          <div className="px-6 py-4 border-t bg-white flex justify-between items-center text-black">
            ...
            {exporter.kyc.status.toLowerCase() === "pending" && onSubmit && (
